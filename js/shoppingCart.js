...
for(let quantity of quantities){
    quantity.addEventListener("change", total);
}
...
function validate(){
    ...
    for(let input of inputs){
        ...
        if (tel.value.length != 9) alert("Phone number should be 9 digits");
        if (zip.value.length != 5) alert("Zip code should be 5 digit");
        if (email.value.indexOf(".") == -1 || email.value.indexOf("@") == -1) alert("Your email address is missing '@' or '.'");
    }
}